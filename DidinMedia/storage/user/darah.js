  const fs = require('fs')
  let _darahOrg = JSON.parse(fs.readFileSync('./DidinMedia/storage/user/darah.json'))

  const addInventoriDarah = (sender, darah) => {
        const obj = {id: sender, healt: darah}
         _darahOrg.push(obj)
        fs.writeFileSync('./DidinMedia/storage/user/darah.json', JSON.stringify(_darahOrg))
   }
