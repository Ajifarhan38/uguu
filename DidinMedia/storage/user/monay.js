  const fs = require('fs')
  let _monayOrg = JSON.parse(fs.readFileSync('./DidinMedia/storage/user/monay.json'))
  let monayAwal = global.limitawal.monayawal
  const addInventoriMonay = (sender) => {
        const obj = {id: sender, monay: monayAwal}
         _monayOrg.push(obj)
        fs.writeFileSync('./DidinMedia/storage/user/monay.json', JSON.stringify(_monayOrg))
   }
